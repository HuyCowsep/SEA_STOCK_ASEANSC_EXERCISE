...
import { getExchangeCache } from "./polling";
...
export function startMatchingEngine(io: Server) {
  ...
  engineInterval = setInterval(async () => {
    try {
      ...
      }).sort({ createdAt: 1 }); // lệnh cũ nhất xử lý trước
...
async function tryMatchOrder(order: IOrder, cache: Record<string, Map<string, Record<string, unknown>>>, io: Server) {
  ...
  if (order.orderType === "ATO" || order.orderType === "ATC" || order.orderType === "MP") {
    ...
    if (closePrice <= 0) return; // chưa có giá → skip

    await matchFull(order, closePrice, io);
    return;
  }

  // Lệnh LO — so sánh giá đặt vs giá bid/offer
  if (order.side === "buy") {
    ...
    if (offerPrice <= 0 || offerVol <= 0) return; // chưa có giá bán → skip
    ...
    if (bidPrice <= 0 || bidVol <= 0) return; // chưa có giá mua → skip
